      ...
      <div className="max-w-4xl mx-auto">
        ...
        {tokenExpired && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-6">
            <p className="text-yellow-800 font-semibold">Token Expired</p>
            <p className="text-yellow-700 text-sm">
              Your access token has expired. It will be refreshed automatically on next API call.
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <dl className="space-y-3">
              <div>
                ...
                <dd className="text-gray-900 font-medium">{session.user?.name || "N/A"}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-500">User ID</dt>
                <dd className="text-gray-900 font-medium">{session.user?.id}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-500">Email</dt>
                <dd className="text-gray-900 font-medium">{session.user?.email}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-500">Culture</dt>
                <dd className="text-gray-900 font-medium">{session.user?.culture}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <dl className="space-y-3">
              <div>
                ...
                <dd className="text-gray-900 font-medium">{session.organization?.id}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-500">EID (Enterprise ID)</dt>
                <dd className="text-gray-900 font-medium">{session.organization?.enterpriseId}</dd>
              </div>
              {session.organization?.stackKey && (
                <div>
                  <dt className="text-sm text-gray-500">Stack</dt>
                  <dd className="text-gray-900 font-medium">{session.organization.stackKey}</dd>
                </div>
              )}
              {session.organization?.region && (
                <div>
                  <dt className="text-sm text-gray-500">Region</dt>
                  <dd className="text-gray-900 font-medium">{session.organization.region}</dd>
                </div>
              )}
            </dl>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 md:col-span-2">
            ...
            <dl className="space-y-3">
              ...
              <div>
                ...
                <dd className="text-gray-900 font-mono text-sm bg-gray-50 p-2 rounded">
                  {session.tokenExpiresAt ? new Date(session.tokenExpiresAt).toLocaleString() : "N/A"}
                </dd>
              </div>
            </dl>
          </div>
        ...
        <div className="mt-6 bg-white rounded-lg shadow-md p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Session Data (Debug)</h2>
            ...
            {JSON.stringify(
              {
                ...
                tokenExpiresAt: session.tokenExpiresAt ? new Date(session.tokenExpiresAt).toISOString() : null,
